...
document.addEventListener('click', function(ev){
  ...
  if (ps){
    var pr = PRESETS[Number(ps.getAttribute('data-preset'))];
    if (pr){
      addEntry({id:String(Date.now()), name:pr.name, portion:pr.portion||'',
        kcal:pr.kcal, carb:pr.carb, protein:pr.protein, fat:pr.fat});
      persist(); renderInput();
    }
    return;
  }
